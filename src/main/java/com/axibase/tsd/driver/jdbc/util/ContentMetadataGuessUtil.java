...
//	}
//
//	public static Pair<String, String[]> findFieldsForTableName(String table) {
////		if (hasWildcard(table)) {
////			throw new IllegalArgumentException("Cannot process generate default schema for query with wildcards");
////		}
////		int endOfSelect = StringUtils.indexOfIgnoreCase(table, SELECT);
////		if (endOfSelect == -1) {
////			throw new IllegalArgumentException("Query should contain SELECT statement");
////		}
////		int startOfFrom = StringUtils.indexOfIgnoreCase(table, FROM, endOfSelect);
////		if (startOfFrom == -1) {
////			throw new IllegalArgumentException("Query should contain datasource");
////		}
//		String fieldNamesGroup = table.substring(endOfSelect + SELECT.length(), startOfFrom).trim();
//		String[] fieldNames = fieldNamesGroup.split("\\s*,\\s*");
//
//		final int endOfFrom = startOfFrom + FROM.length();
//		final String tableName = findTableName(table, endOfFrom);
//
