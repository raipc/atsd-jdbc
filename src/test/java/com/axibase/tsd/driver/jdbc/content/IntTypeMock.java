...
public class IntTypeMock extends AtsdProperties {
	private static final Logger logger = LoggerFactory.getLogger(IntTypeMock.class);
	private static final String CONTEXT_START = "{";
	private static final String TMI_TABLE = "jdbc.driver.test.metric.int";
	private static final String TMI_JSON_SCHEMA = String.format("/json/%s.jsonld", TMI_TABLE);
	private SdkProtocolImpl protocolImpl;
	private boolean isDefaultStrategy;

	@Before
	public void setUp() throws Exception {
		final ContentDescription cd = new ContentDescription(HTTP_ATSD_URL, SELECT_ALL_CLAUSE + TMI_TABLE, LOGIN_NAME,
				LOGIN_PASSWORD, null);
	...
	public void testIntegerType() throws Exception {
		...
		try (IStoreStrategy storeStrategy = PowerMockito
			...
			fetch(storeStrategy, String.format("/csv/%S.csv", TMI_TABLE), 1);
		}
	}

	private String getSchema() throws IOException {
		try (final InputStream is = this.getClass().getResourceAsStream(TMI_JSON_SCHEMA);
