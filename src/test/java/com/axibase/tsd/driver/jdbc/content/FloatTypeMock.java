...
public class FloatTypeMock extends AtsdProperties {
	private static final Logger logger = LoggerFactory.getLogger(FloatTypeMock.class);
	private static final String CONTEXT_START = "{";
	private static final String TMF_TABLE = "jdbc.driver.test.metric.float";
	private static final String TMF_JSON_SCHEMA = String.format("/json/%s.jsonld", TMF_TABLE);
	private SdkProtocolImpl protocolImpl;
	private boolean isDefaultStrategy;

	@Before
	public void setUp() throws Exception {
		final ContentDescription cd = new ContentDescription(HTTP_ATSD_URL, SELECT_ALL_CLAUSE + TMF_TABLE, LOGIN_NAME,
				LOGIN_PASSWORD, null);
	...
	public void testFloatType() throws Exception {
		...
		try (IStoreStrategy storeStrategy = PowerMockito
			...
			fetch(storeStrategy, String.format("/csv/%S.csv", TMF_TABLE), 1);
		}
	}

	private String getSchema() throws IOException {
		try (final InputStream is = this.getClass().getResourceAsStream(TMF_JSON_SCHEMA);
