...
public class ShortTypeMock extends AtsdProperties {
	private static final Logger logger = LoggerFactory.getLogger(ShortTypeMock.class);
	private static final String CONTEXT_START = "{";
	private static final String TMS_TABLE = "jdbc.driver.test.metric.short";
	private static final String TMS_JSON_SCHEMA = String.format("/json/%s.jsonld", TMS_TABLE);
	private SdkProtocolImpl protocolImpl;
	private boolean isDefaultStrategy;

	@Before
	public void setUp() throws Exception {
		final ContentDescription cd = new ContentDescription(HTTP_ATDS_URL, SELECT_ALL_CLAUSE + TMS_TABLE, LOGIN_NAME,
				LOGIN_PASSWORD, null);
	...
	public void testShortType() throws Exception {
		...
		try (IStoreStrategy storeStrategy = PowerMockito
			...
			fetch(storeStrategy, String.format("/csv/%S.csv", TMS_TABLE), 1);
		}
	}

	private String getSchema() throws IOException {
		try (final InputStream is = this.getClass().getResourceAsStream(TMS_JSON_SCHEMA);
